...
import { Heart, ChatCircle, Plus, X, Image as ImageIcon, VideoCamera, PaperPlaneTilt } from "@phosphor-icons/react";
...
export default function Feed() {
  ...
  const [form, setForm] = useState({ media_type: "image", media_url: "", caption: "" });
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      setOpen(false);
      setForm({ media_type: "image", media_url: "", caption: "" });
      load();
    } catch (e) {}
    setPosting(false);
  };
      ...
      {open && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center p-3" onClick={() => setOpen(false)}>
          <form onClick={(e) => e.stopPropagation()} onSubmit={submit}
            ...
            <Button type="submit" disabled={posting} className="w-full mt-4" data-testid="post-submit">
              {posting ? "Posting..." : "Share +3 pts"}
            </Button>
          </form>
        </div>
      )}
